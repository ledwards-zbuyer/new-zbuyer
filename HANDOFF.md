...
- **Estimate RANGES view (2026-08-03, alternate — `?ranges=1` on the report):**
  ...
  arrival; the dotted end-risers added earlier that day were REMOVED same day); the untouched handle parks at the union's CENTER with the headline
...
- **Z-beat sphere widget (2026-08-02, simplified 2026-08-03):** `z-beat-tap.html` —
  ...
  `mockups/shots/zsf/` — 98 frames/colorway captured headless at 640px via
  ...
  adaptive-256). GOTCHAS: (1) `--virtual-time-budget` does NOT step this rAF
  loop — every budget renders the same pose (the 392-frame first attempt was 4
  identical sets); the `?t` freeze is the only reliable frame hook. (2) Capture
  at ≥~520px: below Chrome's minimum layout-viewport width the page lays out
  wider than the bitmap and the ball lands off-center (the first shipped export
  was off-center for this reason, at 480px). (3) Bare mode originally kept the
  standalone 347px ball cap, which made big capture frames render a small ball
  (72% fill) — the cap is now standalone-only; bare fills 96% at any size. The 2026-08-03 simplification REMOVED the press-and-hold status-word
